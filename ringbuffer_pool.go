...
//   - Significantly faster than mutex-based implementations
//   - Optimized for maximum performance with lock-free operations
//   - Supports both lock-free and mutex-based ring buffers
//
...
type RingBufferWorkerPool struct {
	...
	taskQueue    *LockFreeRingBuffer[Task] // 8 bytes - hot path for enqueue/dequeue
...
// NewRingBufferWorkerPool creates a lock-free ring buffer-based worker pool.
//
...
func NewRingBufferWorkerPool(maxWorkers int, queueSize int) *RingBufferWorkerPool {
	...
	pool := &RingBufferWorkerPool{
		...
		taskQueue:  NewLockFreeRingBuffer[Task](queueSize), // Lock-free ring buffer main queue
		ctx:        ctx,
		cancel:     cancel,
	}
...
// NewRingBufferWorkerPoolWithMutex creates a mutex-based ring buffer worker pool for comparison.
// This version uses the mutex-based ring buffer implementation.
//
// Parameters:
	...
	for {
		...
		default:
			...
			if task, ok := p.taskQueue.Dequeue(); ok {
				...
				// Simple round-robin delivery with blocking send
				// This ensures we never lose tasks or need to re-queue
				worker := p.workers[workerIndex&workerMask]
				worker.taskChan <- task // Block until worker can accept
				workerIndex = (workerIndex + 1) & workerMask
			} else {
...
func (p *RingBufferWorkerPool) Wait() {
	...
	for {
		...
		if runningTasks == 0 && queueLen == 0 {
			...
			if !hasWorkerTasks {
				// Small delay to handle race condition where dispatcher just delivered
				// a task but worker hasn't picked it up yet
				...
				if runningTasks == 0 && queueLen == 0 && !hasWorkerTasks {
					break
				}
			}
		}
...
//
//	pool := NewRingBufferWorkerPool(64, 128)
//	defer pool.Release()
func (p *RingBufferWorkerPool) Release() {
	p.cancel()

	// Wait for all currently running tasks to complete with timeout to avoid race condition
	// This prevents workers from decrementing runningTasks after we reset it
	timeout := time.After(5 * time.Second)
	for atomic.LoadUint64(&p.runningTasks) > 0 {
		select {
		case <-timeout:
			// Force exit after timeout to avoid infinite wait
			goto cleanup
		default:
			runtime.Gosched()
		}
...
cleanup:
	// Clear the main queue with safety limit
	drainCount := 0
	maxDrain := 10000 // Prevent infinite loop
	for drainCount < maxDrain {
	...
	for _, worker := range p.workers {
		...
		drainStart := time.Now()
		for time.Since(drainStart) < time.Second {
			select {
